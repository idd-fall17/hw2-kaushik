...
public abstract class SimplePicoPro extends SimpleBoard {
    ...
    // Row IDs
    Map<Integer, Integer> layoutObjects;
    public SimplePicoPro() {
        try {
            ...
            // Initialize Map with correct row values
            layoutObjects = new HashMap<Integer, Integer>();
            layoutObjects.put(0, R.id.row1);
            layoutObjects.put(1, R.id.row2);
            layoutObjects.put(2, R.id.row3);
            layoutObjects.put(3, R.id.row4);
            layoutObjects.put(4, R.id.row5);
            layoutObjects.put(5, R.id.row6);
            layoutObjects.put(6, R.id.row7);
    ...
    void printCharacterToScreen(char c) {
        ...
        if(editText != null) {
            if (c == '^') {
                editText.getText().delete(editText.getText().length()-2, editText.getText().length() -1);
            } else {
    ...
    void changeRow(int rowID) {
        if (activity == null) {
            Log.e(TAG,"printChar: activity is null");
            return;
        }

        // help from: https://stackoverflow.com/questions/28269837/android-layout-params-change-only-width-and-height
        // null all other rowID highlights
        for (int row: layoutObjects.keySet()) {
            // Set the background color as white
            activity.findViewById(layoutObjects.get(row)).setBackgroundColor(Color.parseColor("#ffffff"));
        }
        // just turn on the one selected
        activity.findViewById(layoutObjects.get(rowID)).setBackgroundColor(Color.parseColor("#00bfff"));

        //activity = (View)findViewById(R.id.nutrition_bar_filled);

    }
    /*
    void toggleCharOnScreen(char c) {
        if (activity == null) {
            Log.e(TAG,"printChar: activity is null");
            return;
        }
